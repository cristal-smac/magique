 ...
 */
public strictfp class InstructionIO
    implements Constants
{
    ...
    {
        /**
         * The index of the referenced target in the instruction array.
         */
    ...
    /**
     * Input stream to read instructions from.
     */
    private DataInput in;

    /**
     * Constant pool being used to verify/read instructions.
     */
        ...
        throws IOException, ClassFileFormatException
    {
        ...
        for (int start = 0; start < length; /*increment in loop body*/) {
            ...
            case opc_dconst_1:
                instructions[start] =
                    new PushLiteralInstruction((double)1);
                break;
            case opc_bipush:
                instructions[start] =
                    new PushLiteralInstruction(in.readByte());
                extra = 1;
                break;
            case opc_sipush:
                instructions[start] =
                    new PushLiteralInstruction(in.readShort());
                extra = 2;
                break;
            case opc_ldc:
                instructions[start] =
                    new PushLiteralInstruction(cp.getEntryAt
                                               (in.readUnsignedByte()), false);
                extra = 1;
                break;
            case opc_ldc_w:
                instructions[start] =
                    new PushLiteralInstruction(cp.getEntryAt
                                               (in.readUnsignedShort()),false);
                extra = 2;
                break;
            case opc_ldc2_w:
                instructions[start] =
                    new PushLiteralInstruction(cp.getEntryAt
                                               (in.readUnsignedShort()), true);
                extra = 2;
                break;
            case opc_iload:
            case opc_lload:
            ...
            case opc_ret:
                if (wide) {
                    wide = false;
                    instructions[start] =
                        new LocalVarInstruction(opcode,
                                                in.readUnsignedShort());
                    extra = 3; // count wide prefix
                }
                else {
                    instructions[start] =
                        new LocalVarInstruction(opcode, in.readUnsignedByte());
                    extra = 1;
                }
                break;
            case opc_iload_0:
            ...
            case opc_iinc:
                if (wide) {
                    wide = false;
                    instructions[start] =
                        new IincInstruction(in.readUnsignedShort(),
                                            in.readShort());
                    extra = 5; // count wide prefix
                }
                else {
                    instructions[start] =
                        new IincInstruction(in.readUnsignedByte(),
                                            in.readByte());
                    extra = 2;
                }
                break;
            ...
            case opc_ifnonnull:
                {
                    ...
                    if (target < start) {
                        instructions[start] =
                            new JumpInstruction(opcode, instructions[target],
                                                "InstructionIO");
                    }
                    else {
                        instructions[start] =
                            new JumpInstruction(opcode,
                                                new ForwardReference(target),
                                                "InstructionIO");
                        forwards.add(instructions[start]);
                    }
                    extra = 2;
                }
                break;
            ...
            case opc_putfield:
                instructions[start] =
                    new FieldRefInstruction
                        (opcode,
                         (FieldRefConstant)
                         cp.getEntryAt(in.readUnsignedShort()));
                extra = 2;
                break;
            ...
            case opc_invokeinterface:
                {
                    ...
                    method = (InterfaceConstant)
                        cp.getEntryAt(in.readUnsignedShort());
                    nargs = in.readUnsignedByte();
                    in.readUnsignedByte();

                    instructions[start] =
                        new InvokeinterfaceInstruction(method, nargs);
                    extra = 2 + 1 + 1;
                }
                break;
            case opc_new:
            ...
            case opc_instanceof:
                instructions[start] =
                    new ClassRefInstruction
                        (opcode,
                         (ClassConstant)cp.getEntryAt(in.readUnsignedShort()));
                extra = 2;
                break;
            case opc_newarray:
                instructions[start] =
                    new NewarrayInstruction((byte)in.readUnsignedByte());
                extra = 1;
                break;
            case opc_multianewarray:
                instructions[start] =
                    new MultiarrayInstruction
                        ((ClassConstant)
                         cp.getEntryAt(in.readUnsignedShort()),
                         in.readUnsignedByte());
                extra = 3;
            ...
            default:
                throw new ClassFileFormatException("Invalid opcode: "
                                                   + opcode);
            }
        ...
        throws ClassFileFormatException
    {
        class MyAccessorTransformer implements AccessorTransformer
        {
                ...
                throws BadAccessorException
            {
                ...
                if (instructions[index] == null) {
                    throw new BadAccessorException("no instruction at "
                                                   + index);
                }
        ...
        for (Iterator iter = forwards.iterator(); iter.hasNext();) {
            AccessorContainer instruction = (AccessorContainer)iter.next();

            try {
                instruction.transformAccessors(transformer);
            }
            catch (BadAccessorException e) {
                throw new ClassFileFormatException("Forward reference not "
                                                   + "resolvable.");
            }
        }
    }
    ...
    {
        //for (Iterator e = switches.iterator(); e.hasNext(); ) {
        //((SwitchInstruction)e.next()).selectSwitchType();
        //    }
    }
     ...
     */
    private int readTableSwitch(int start)
        throws IOException
    {
        int extra = 0;
        ...
        instructions[start] = new SwitchInstruction(deflab, matches, targets);
        extra += 4 + 4 + 4 + 4*count;

        if (addForward) {
            forwards.add(instructions[start]);
        }
        switches.add(instructions[start]);
     ...
     */
    private int readLookupSwitch(int start)
        throws IOException
    {
        int extra = 0;
