...
public class OptionsFiles {
   ...
   */
	public Color[] getColors() {
		...
		try {
			option = options.getString("BackgroundColor"); colors[0] = parseColorList(option);
			option = options.getString("AgentColor"); colors[1] = parseColorList(option);
			option = options.getString("FontColor"); colors[2] = parseColorList(option);
			option = options.getString("LinkColor"); colors[3] = parseColorList(option);
			option = options.getString("DirectLink"); colors[4] = parseColorList(option);			
			option = options.getString("SelectionColor"); colors[5] = parseColorList(option);
		}
		catch (Throwable mre) {
   ...
   */
	public int[] getMainBound() {
		...
		try {
			option = options.getString("MainBound"); bounds = parseIntList(option);
		}
		catch (Throwable mre) {
			bounds[0] = 0; bounds[1] = 0;
			bounds[2] = screen.width; bounds[3] = screen.height;
		}
		return bounds;
	}
   ...
   */
	public int[] getComputersBound() {
		int[] bounds = new int[4];
		try {
			option = options.getString("ComputersBound"); bounds = parseIntList(option);
                        if (bounds[0]>screen.width-20) throw new Throwable();
                        if (bounds[1]>screen.height-15) throw new Throwable();
		}
		catch (Throwable mre) {
			bounds[0] = 3; bounds[1] = screen.height/2;
			bounds[2] = 175; bounds[3] = screen.height/2-65;
		}
		return bounds;
	}
		...
		int[] bounds = new int[4];
		try {
                        option = options.getString("ExecutionBound"); bounds = parseIntList(option);
                        if (bounds[0]>screen.width-15) throw new Throwable();
                        if (bounds[1]>screen.height-15) throw new Throwable();
		}
		catch (Throwable mre) {
                        bounds[0] = 0; bounds[1] = 0;
                        bounds[2] = 400; bounds[3] = 400;
		}
		return bounds;
	}
  /**
   ...
   */
	public int[] getClassesBound() {
		int[] bounds = new int[4];
		try {
			option = options.getString("ClassesBound"); bounds = parseIntList(option);
                        if (bounds[0]>screen.width-20) throw new Throwable();
                        if (bounds[1]>screen.height-15) throw new Throwable();
		}
		catch (Throwable mre) {
			bounds[0] = screen.width-207; bounds[1] = screen.height/2; 
			bounds[2] = 185; bounds[3] = screen.height/2-65;
		}
		return bounds;
	}
   ...
   */
	public int[] getPropertiesBound() {
		int[] bounds = new int[4];
		try {
			option = options.getString("PropertiesBound"); bounds = parseIntList(option);
                        if (bounds[0]>screen.width-20) throw new Throwable();
                        if (bounds[1]>screen.height-15) throw new Throwable();
		}
		catch (Throwable mre) {
			bounds[0] = (screen.width-300)/2; bounds[1] = screen.height-200;
			bounds[2] = 300; bounds[3] = 135;
		}
		return bounds;
	}
   ...
   */
	public int[] getTreeBound() {
		int[] bounds = new int[4];
		try {
			option = options.getString("TreeBound"); bounds = parseIntList(option);
                        if (bounds[0]>screen.width-15) throw new Throwable();
                        if (bounds[1]>screen.height-15) throw new Throwable();
		}
		catch (Throwable mre) {
			bounds[0] = 0; bounds[1] = 0;
			bounds[2] = screen.width; bounds[3] = screen.height-45;
		}
		return bounds;
	}
		...
		int[] bounds = new int[4];
		try {
                        option = options.getString("FluxBound"); bounds = parseIntList(option);
                        if (bounds[0]>screen.width-20) throw new Throwable();
                        if (bounds[1]>screen.height-20) throw new Throwable();
		}
		catch (Throwable mre) {
			bounds[0] = 0; bounds[1] = 0;
                        bounds[2] = 400; bounds[3] = 200;
		}
		return bounds;
	}
