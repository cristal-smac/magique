...
public class TreeOrganiser {
   ...
   */
 	private int ecartementX;
 	
  /**
   * The vertical shift between two nodes
   */
 	private int ecartementY;
   ...
   */
 	public TreeOrganiser(GraphicNode root, int Marge, int ecartY, int ecartX) {
   	Tree = new Vector(); Root = root;
   	marge = Marge; decalage = marge;
   	ecartementX = ecartX; ecartementY = ecartY;	
 	}
  	
  /**
   * Returns the max width of this tree
   */
 	public int widthMax () {return decalage;}

  /**
   * Returns the max height of this tree
   */
  public int heightMax() {return (Tree.size()+2)*ecartementY;}
  ...
  private void add(GraphicNode Node, int high) {
   	...
   	if ( high == 0 || high >= Tree.size() ) {
     	Tree.addElement(stage); positionX = marge;
   	}
   	else {
     	GraphicNode dernier;
   ...
   */
 	private void moveSubTree(GraphicNode Parent, int h, int offset) {
   	if (Parent.isVisible()) {
   		...
   		if ( h > compteur ) {
     		moveStage(Parent, h, offset); compteur = h;
   		}      
   		if (!VChild.isEmpty()) {
   ...
   */
 	private void moveTree(GraphicNode Parent, int h, int offset) {
 	  moveSubTree(Parent, h, offset); compteur = h;
 	}
   ...
   * Organises this tree
   */
  public void Organise() {Organise1(Root,0); Organise2(Root,0);}
