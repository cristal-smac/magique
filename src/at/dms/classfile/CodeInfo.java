...
public class CodeInfo extends Attribute
{
                    ...
                    LocalVariableInfo[] localVariables)
    {
        ...
        this.handlers = (handlers == null
                         ? new HandlerInfo[0] : handlers);

        this.attributes =
            new AttributeList(lineNumbers != null
                              ? new LineNumberTable(lineNumbers) : null,
                              localVariables != null
                              ? new LocalVariableTable(localVariables) : null);
    }
        ...
        for (int i = 0; i < instructions.length; i++) {
            if (this.instructions[i] instanceof AccessorContainer) {
                AccessorContainer container =
                    (AccessorContainer)this.instructions[i];
                container.transformAccessors(transformer);
            }
        }
    ...
    {
        if (codeLength == -1) {
            throw new InternalError("code length not yet computed");
        }

        int size = (2 + 4 + 2 + 2 + 4 + codeLength
                    + 2 + 2 + 8 * handlers.length + attributes.getSize());
    ...
    public LocalVariableInfo[] getLocalVariables()
    {
        ...
        return attr == null
            ? null : ((LocalVariableTable)attr).getLocalVariables();
    }
    ...
    /**
     * Returns the number of locals vars used in this method (including
     * parameters).
     */
