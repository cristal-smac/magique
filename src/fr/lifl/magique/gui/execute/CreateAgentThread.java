...
class CreateAgentThread extends Thread {
	private ConfigReader reader; private Hashtable computers ;
	private Deployement deployer;private Configuration conf;
	...
	public void run () {
		...
		if (!res.booleanValue()) {
		    ...
		    deployer.kill(conf.getName());return;
		}
		...
		try {
		    ...
		    deployer.addError("L'agent créé est:"+res);
		    try {
	    ...
	    for (Enumeration e=agents.elements();e.hasMoreElements();) {
		GraphicAgent graph_agent=(GraphicAgent) e.nextElement();  			String computer=this.getComputer(graph_agent.getComputer(),computers);	       
			...
			if(skillargs.size()==0)
{
	   ...
	   //AJOUT DES LIENS !!!
		if (graph_agent.GetParent() != null) {
		    ...
		    param[0]=((GraphicAgent) graph_agent.GetParent()).getName()+"@"+getComputer(((GraphicAgent)graph_agent.GetParent()).getComputer(),computers);
