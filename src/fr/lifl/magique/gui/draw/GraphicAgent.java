...
public class GraphicAgent extends GraphicNode {
     ...
     */
    private final Vector Initialisers = new Vector();
    private boolean isGraphic = true;
    private final Vector skills = new Vector();
     ...
     */
    public String getInitialiserFunction(int i) {
        return ((Initialiser) Initialisers.elementAt(i)).getFunction();
    }
     ...
     */
    public String getInitialiserArgument(int i) {
     ...
     */
    public int getInitialisersNumber() {
...
//    Initialisers.removeAllElements();
    ...
    public void addInitialiser(String func, String arg) {
        ...
        for (int i = 0; i < Initialisers.size(); i++) {
            if (((Initialiser) Initialisers.elementAt(i)).getFunction().equals(func)) {
                ((Initialiser) Initialisers.elementAt(i)).setArgument(arg);
                change = true;
            }
        }
    ...
    public void setMetrics(FontMetrics metric) {
        police = metric;
        setDimension(new Dimension(rect.width, rect.height));
    }
     ...
     */
    public void setIconVisible(boolean IconActive) {
        isIconVisible = IconActive;
    }
    ...
    public void draw(Graphics g1, boolean selected, Color AColor, Color FColor, Color SColor, Color BColor) {
        if (isVisible) {
            ...
            // Line Selector
            g.setColor(SColor);
            g.fillOval(Center.x - 10, rect.y - 10, 20, 22);
            g.setColor(BColor);
            g.fillOval(Center.x - 4, rect.y - 8, 8, 7);
    ...
    public boolean equals(GraphicAgent compare) {
        return compare.getName().equals(name) && compare.getComputer().equals(computer) && compare.getClasse().equals(classe);
    }
