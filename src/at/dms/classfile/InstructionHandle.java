...
class InstructionHandle extends AbstractInstructionAccessor
{
    ...
    /**
     * Notifies this handle that is has been attached to the specified
     * container.
     */
     ...
     */
    final boolean checkInstruction(CodeEnv env, int curStack)
        throws ClassFileFormatException
    {
        curStack += insn.getStack();
        if (curStack < 0) {
            System.err.println(">>>>>>>>" + curStack + " / "
                               + insn.getStack());
            dump();
        ...
        if (stackHeight != Integer.MIN_VALUE) {
            if (stackHeight != curStack) {
                ...
                throw new ClassFileFormatException
                    ("@" + index + "(" + OpcodeNames.getName(insn.getOpcode())
                     + "): stack height different: "
    ...
    /**
     * Sets the position of this instruction and return true if address is
     * final.
     */
    ...
    /**
     * The stack height when this instruction is reached by some execution
     * path, after execution of this instruction (Integer.MIN_VALUE:
     * unreachable)
     */
